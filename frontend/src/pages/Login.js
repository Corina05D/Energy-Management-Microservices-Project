...
const Login = () => {
    ...
    const onButtonClicked = async () => {
        ...
        var userCanLogin = await axios.post(`http://localhost:8081/User/IsUserVerified`, params)
        if(userCanLogin.data){
            var id = await axios.post(`http://localhost:8081/User/GetUserId`, params)
            var email = emailValue
            var userRole = await axios.post(`http://localhost:8081/User/UserRole/${email}`)
